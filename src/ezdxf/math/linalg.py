...
class SVDecomposition:
    ...
    def __init__(self, A: Iterable[Iterable[float]]):
        self.u = copy_float_matrix(A)
        self.v = Matrix(shape=(self.nrows, self.ncols)).matrix
        self.w = [0.0] * self.nrows
        self._decompose()
        self._reorder()
        self.tsh = 0.5 * math.sqrt(self.nrows + self.ncols + 1.0) * self.w[0] * sys.float_info.epsilon
    ...
    def _thresh(self, t: float):
        return t if t >= 0. else 0.5 * math.sqrt(self.nrows + self.ncols + 1.) * self.w[0] * sys.float_info.epsilon

    def solve_vector(self, B: Iterable[float], thresh: float = -1) -> List[float]:
        """
        Solves the linear equation system given by the nxn Matrix A . x = B,
        right-hand side quantities as vector B with n elements.
        ...
        w = self.w
        v = self.v
        tsh = self._thresh(thresh)

        tmp = [sum(u[i][j] * b[i] for i in range(m)) / w[j] if w[j] > tsh else 0.0 for j in range(n)]
        x = [sum(v[i][j] * tmp[j] for j in range(n)) for i in range(n)]
        return x
    ...
    def rank(self, thresh: float = -1.0) -> int:
        tsh = self._thresh(thresh)
        return sum(int(w > tsh) for w in self.w)

    def nullity(self, thresh: float = -1.0) -> int:
        tsh = self._thresh(thresh)
        return sum(int(w <= tsh) for w in self.w)

    def range(self, thresh: float = -1.0) -> MatrixData:
        tsh = self._thresh(thresh)
        w = self.w
        u = self.u
        range_ = [[0.0] * self.rank(thresh) for _ in range(self.nrows)]
        nr = 0
    ...
    def nullspace(self, thresh: float = -1.0) -> MatrixData:
        tsh = self._thresh(thresh)
        w = self.w
        n = self.ncols
        nullspace_ = [[0.0] * n for _ in range(self.nullity(thresh))]
        nn = 0
        for j in range(n):
            if w[j] <= tsh:
                for jj in range(n):
                    nullspace_[jj][nn] = self.v[jj][j]
                nn += 1
        return nullspace_

    def _inv_condition(self):
        w = self.w
        n = self.ncols
        return 0.0 if w[0] <= 0. or w[n - 1] <= 0.0 else w[n - 1] / w[0]
    ...
    def _decompose(self) -> None:
        ...
        for k in range(n - 1, -1, -1):
            for its in range(30):
                ...
                if its == 29:
                    raise ArithmeticError("No convergence in 30 svdcmp iterations")
                x = w[l]
                nm = k - 1
                y = w[nm]
                g = rv1[nm]
    ...
    def _reorder(self) -> None:
        ...
        su = [0.0] * m
        sv = [0.0] * n
        while inc <= n:  # do { ... }} while loop!!!
            inc *= 3
            inc += 1
        # while (inc <= n) !!!

        while inc > 1:  # do { ... } while loop!!!
            inc /= 3
...
def _pythag(a: float, b: float) -> float:
    ...
    if absa > absb:
        ...
        return 0.0 if absb == 0.0 else absb * math.sqrt(1.0 + (absa / absb) ** 2)
