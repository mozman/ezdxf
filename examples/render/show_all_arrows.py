...
for index, name in enumerate(sorted(ezdxf.ARROWS.__all_arrows__)):
    ...
    cp1 = msp.add_arrow(
        name, insert=(0, y), size=1, rotation=180, dxfattribs={"color": 7}
    )
    # right side ->| is the base orientation
    cp2 = msp.add_arrow(
        name, insert=(4, y), size=1, rotation=0, dxfattribs={"color": 7}
    )
    ...
    add_connection_point(
        msp.add_arrow_blockref(name, insert=(7.5, y), size=0.3, rotation=135)
    )
    add_connection_point(
