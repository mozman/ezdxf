...
def repair_leica_disto_r12(dwg):
    ...
    The base file format is DXF R12, but with subclass marker like (100, AcDbEntity), which were introduced with
    DXF R13. ezdxf does not ignore this subclass marker and creates this subclasses, which are not seen by the legacy
...
def is_leica_disto_r12(dwg):
    if dwg.dxfversion > 'AC1009':
        return False
    for entity in dwg.modelspace():  # tests only the first entity in model space
        if len(entity.tags.subclasses) > 1:  # if DXF R12 files have subclasses
            return True
        else:
            return False
