...
def doc():
    ...
    msp.add_blockref(
        "Test1", (20, 10), dxfattribs={"xscale": 2}
    )  # yscale and zscale
    return d
...
def test_02_explode_blockrefs(doc, msp, entitydb):
    ...
    assert (
        blockref.is_alive is False
    ), "Exploded block reference should be destroyed."
...
def test_06_skipped_entities_callback(doc, msp):
    ...
    blk.add_line((0, 0), (1, 0))
    blkref = msp.add_blockref("test_block", insert=(0, 0)).place(
        (0, 0), (1, 2, 3)
    )
    skipped_entities = []
    ...
    entities = list(
        blkref.virtual_entities(skipped_entity_callback=on_entity_skipped)
    )
...
def _get_transformed_curve(
    ...
    if is_arc:
        blk.add_arc(
            (0, 0), radius=1, start_angle=0, end_angle=math.degrees(math.pi / 2)
        )
    else:
        blk.add_ellipse(
            (0, 0),
    ...
    blk.add_point((1, 0))
    blk.add_point((0, 1))
    block_ref = doc.modelspace().add_blockref(
        "block",
        insert=(0, 0),
        dxfattribs={
            "xscale": scale_factors.x,
            "yscale": scale_factors.y,
            "zscale": scale_factors.z,
