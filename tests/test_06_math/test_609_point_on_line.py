...
def test_distance_point_horiz_line():
    ...
    assert distance_point_line(Vec2(5, 2), start, end) == 2
    assert distance_point_line(Vec2(5, -3), start, end) == 3
    assert distance_point_line(Vec2(0, 0), start, end) == 0


def test_distance_point_vertical_line():
    start, end = Vec2(0, 0), Vec2(0, 10)
    assert distance_point_line(Vec2(2, 0), start, end) == 2
    assert distance_point_line(Vec2(-3, 0), start, end) == 3
    assert distance_point_line(Vec2(0, 0), start, end) == 0
