...
def test_priority_order(space):
    assert list(e.priority for e in space.priority()) == sorted(NUMBERS, reverse=True), 'highest priority first'


def test_zorder(space):
    assert list(e.priority for e in space.zorder()) == sorted(NUMBERS), 'lowest priority first'
