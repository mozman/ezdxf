...
def test_wcs_to_ocs():
    ...
    assert is_close_points(
        ocs.wcs_to_ocs((-9.56460754, 8.44764172, 9.97894327)),
        (9.41378764657076, 13.15481838975576, 0.8689258932616031),
        places=6,
    )
    assert is_close_points(
        ocs.wcs_to_ocs((-1.60085321, 9.29648008, 1.85322122)),
        (9.41378764657076, 1.745643639268379, 0.8689258932616031),
        places=6,
    )
    assert is_close_points(
        ocs.wcs_to_ocs((-3.56027455, 9.08762984, 3.85249348)),
        (9.41378764657076, 4.552784531093068, 0.8689258932616031),
        places=6,
    )
    assert is_close_points(
        ocs.wcs_to_ocs((-5.53851623, 8.87677359, 5.87096886)),
...
def test_ocs_to_wcs():
    ...
    assert is_close_points(
        ocs.ocs_to_wcs((9.41378764657076, 1.745643639268379, 0.8689258932616031)),
        (-1.60085321, 9.29648008, 1.85322122),
        places=6,
    )
    assert is_close_points(
        ocs.ocs_to_wcs((9.41378764657076, 4.552784531093068, 0.8689258932616031)),
