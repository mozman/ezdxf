...
- # Selection Module
	...
	- Handles all entities as projected on the xy-plane - no 3D support.
	- Selection methods
		- Inside Window
		...
		- Inside Circle
			- entity bbox inside a circle
		- Crossing Circle
			- entity bbox inside or crossing a circle
		- Inside Polygon
			- entity bbox inside a polygon
		- Crossing Polygon
			- entity bbox inside or crossing a polygon
		- Point
			- point is inside the entity bbox
		- Fence
			- entity bboxes crossing a polyline
		- Cluster
		...
		- Outside or Inverse Selection
			- `Set` operation `difference` of `EntityQuery`: all entities minus select operation
	...
	- #unlikely Extended entity geometry for selection
		...
		- How to handle 2D/3D selections? - ignore 3D, handle entities as projected on the xy-plane
			- 2D selections always in top-view mode? yes - always top-view
